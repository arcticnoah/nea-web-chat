...
// Note: This is identical to the other version of this file except the
// exports here are for ES6 whilst the other is for Node.JS
...
export function checkLength(input, minimumLength, maximumLength) {
...
export function checkEmail(input) {
    ...
    let inputLength = checkLength(input, 5, 128);
    if (inputLength === input) {
        ...
        if (indexOfSeparator > 0) {
            ...
            let localPart = input.substring(0, indexOfSeparator);
            if (/^[\x00-\x7F]*$/.test(localPart) === true) {
                // Local-part is valid
                let domain = input.substring(indexOfSeparator + 1);
...
export function checkUsername(input) {
    input = cleanText(input);
    let inputLength = checkLength(input, 1, 16);
    if (inputLength === input) {
        // Within length range
        return [input, true];
    } else {
...
export function checkPassword(input) {
    input = cleanText(input);
    let inputLength = checkLength(input, 8, 32);
    if (inputLength === input) {
        // Within length range
...
export function checkInviteCode(input) {
    input = cleanText(input);
    let inputLength = checkLength(input, 8, 8);
    if (inputLength === input) {
        // Within range
...
export function checkEmailCode(input) {
    input = cleanText(input);
    let inputLength = checkLength(input, 6, 6);
    if (inputLength === input) {
        // Within range
        if (/^[0-9]+$/.test(input)) {
            // Valid email code
...
// export { cleanText, checkLength, checkEmail }